.PHONY: deps source-build-headed source-build-headless prebuilt-headed-x86_64 prebuilt-headless-x86_64 prebuilt-headed-arm64 prebuilt-headless-arm64 check
...
prebuilt-headed-x86_64:
	...
	url=$$(curl -s https://api.github.com/repos/$(GITHUB_REPO)/releases/tags/latest-x86_64-headed \
		| grep 'browser_download_url' \
		| grep '\.whl' \
		| head -n1 \
		| cut -d '"' -f4); \
	if [ -z "$$url" ]; then echo "No wheel asset found for latest-x86_64-headed"; exit 1; fi; \
	wget -q --show-progress "$$url"; \
	$(PIP) install /tmp/opencv-wheels/$$(basename "$$url")

prebuilt-headless-x86_64:
	@echo "Uninstalling any existing OpenCV packages from the virtual environment..."
	$(PIP) uninstall -y opencv-python opencv-contrib-python opencv-python-headless || true
	@echo "Downloading latest prebuilt opencv-python-headless wheel (x86_64) from GitHub..."
	mkdir -p /tmp/opencv-wheels
	cd /tmp/opencv-wheels && \
	url=$$(curl -s https://api.github.com/repos/$(GITHUB_REPO)/releases/tags/latest-x86_64-headless \
		| grep 'browser_download_url' \
		| grep '\.whl' \
		| head -n1 \
		| cut -d '"' -f4); \
	if [ -z "$$url" ]; then echo "No wheel asset found for latest-x86_64-headless"; exit 1; fi; \
	wget -q --show-progress "$$url"; \
	$(PIP) install /tmp/opencv-wheels/$$(basename "$$url")

prebuilt-headed-arm64:
	@echo "Uninstalling any existing OpenCV packages from the virtual environment..."
	$(PIP) uninstall -y opencv-python opencv-contrib-python opencv-python-headless || true
	@echo "Downloading latest prebuilt opencv-python wheel (arm64) from GitHub..."
	mkdir -p /tmp/opencv-wheels
	cd /tmp/opencv-wheels && \
	url=$$(curl -s https://api.github.com/repos/$(GITHUB_REPO)/releases/tags/latest-arm64-headed \
		| grep 'browser_download_url' \
		| grep '\.whl' \
		| head -n1 \
		| cut -d '"' -f4); \
	if [ -z "$$url" ]; then echo "No wheel asset found for latest-arm64-headed"; exit 1; fi; \
	wget -q --show-progress "$$url"; \
	$(PIP) install /tmp/opencv-wheels/$$(basename "$$url")
...
prebuilt-headless-arm64:
	...
	mkdir -p /tmp/opencv-wheels
	cd /tmp/opencv-wheels && \
	url=$$(curl -s https://api.github.com/repos/$(GITHUB_REPO)/releases/tags/latest-arm64-headless \
		| grep 'browser_download_url' \
		| grep '\.whl' \
		| head -n1 \
		| cut -d '"' -f4); \
	if [ -z "$$url" ]; then echo "No wheel asset found for latest-arm64-headless"; exit 1; fi; \
	wget -q --show-progress "$$url"; \
	$(PIP) install /tmp/opencv-wheels/$$(basename "$$url")
